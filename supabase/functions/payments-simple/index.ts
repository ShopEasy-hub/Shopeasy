...
// Helper to get authenticated user (ONLY for initialization, NOT for verification)
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (path.endsWith('/payments-simple') || path.endsWith('/payments-simple/')) {
      return new Response(
        JSON.stringify({
          ...
          version: '3.0.0',
          message: 'Payment verification works WITHOUT authentication',
        }),
    ...
    if (path.includes('/paystack/initialize') && req.method === 'POST') {
      console.log('💳 [PayStack] Initialize payment request');
      const user = await getAuthUser(req);
      if (!user) {
        console.error('❌ [PayStack] Unauthorized - no valid user');
        return new Response(
      ...
      console.log('🔍 Callback URL:', `${frontendUrl}?payment-callback=true&reference=${reference}`);
    ...
    if (path.includes('/paystack/verify/') && req.method === 'GET') {
      ...
      // Update payment record (using SERVICE ROLE - bypasses RLS)
    ...
    // ============================================
    // FLUTTERWAVE INITIALIZE (Requires Auth)
    // ============================================
    if (path.includes('/flutterwave/initialize') && req.method === 'POST') {
      console.log('💳 [Flutterwave] Initialize payment request');
      const user = await getAuthUser(req);
      if (!user) {
        console.error('❌ [Flutterwave] Unauthorized - no valid user');
        return new Response(
          JSON.stringify({ error: 'Unauthorized' }),
          { status: 401, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      ...
      const response = await fetch('https://api.flutterwave.com/v3/payments', {
        ...
        body: JSON.stringify({
          ...
          amount: amountInNGN,
          currency: currency || 'NGN',
          redirect_url: `${Deno.env.get('FRONTEND_URL') || 'http://localhost:3000'}?payment-callback=true`,
          customer: { email },
          customizations: {
