...
export default function AdminPanel({ appState, onNavigate }: AdminPanelProps) {
    ...
    useEffect(() => {
        ...
        loadAdminData();
    }, [isAdmin, onNavigate]); // Removed onBack from dependencies to prevent infinite loop
    ...
    const loadAdminData = async () => {
        ...
        try {
            ...
            // Extract org from the wrapper (getOrganization returns { org })
            ...
            const subEndDate = appState.subscriptionEndDate || org?.subscription_end_date;
            const subPlan = appState.subscriptionPlan || org?.subscription_plan;
            const trialStart = appState.trialStartDate || org?.trial_start_date;

            console.log('📊 [AdminPanel] Subscription data:');
            console.log('  - Status:', subStatus);
            console.log('  - Plan:', subPlan);
            console.log('  - End date:', subEndDate);
            console.log('  - Trial start:', trialStart);
            console.log('  - Source: appState =', appState.subscriptionStatus, ', org =', org?.subscription_status);
            ...
            setStats({
                ...
                todaySales,
                subscriptionStatus,
                subscriptionPlan: subPlan || 'starter',
                daysUntilExpiry,
            });
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-muted-foreground">
                            System administration for {appState.companyName}
                        </p>
                    </div>
                </div>
                                ...
                                <div className="space-y-3">
                                    {recentActivity.map((activity) => (
                                        <div key={activity.id} className="flex items-start gap-3 pb-3 border-b last:border-0">
                                            <div className={`h-2 w-2 rounded-full mt-2 ${activity.type === 'create' ? 'bg-green-500' :
                        ...
                        <Card>
                            ...
                            <CardContent className="space-y-4">
                                <div className="flex items-center justify-between p-4 border rounded-lg">
                                    <div>
                                        <p>Current Plan: <strong className="capitalize">{stats.subscriptionPlan}</strong></p>
                                        <p className="text-sm text-muted-foreground">
                                            {stats.subscriptionStatus === 'trial'
                                                ? `Trial expires in ${stats.daysUntilExpiry} days`
                                                : `Subscription expires in ${stats.daysUntilExpiry} days`
                                            }
                                        </p>
                                    </div>
