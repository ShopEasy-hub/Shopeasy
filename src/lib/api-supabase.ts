...
export async function getUserProfile(userId?: string) {
    ...
    const { data, error } = await supabase
        .from('user_profiles')
        .select('*, organization:organizations(name, subscription_status, subscription_plan, subscription_end_date, trial_start_date)')
        .eq('id', user.id)
        .single();
...
/**
 * Get current user's access token with enhanced logging and auto-refresh
 */
export async function getAccessToken() {
    console.log('🔐 [getAccessToken] Getting session...');

    const { data: { session }, error } = await supabase.auth.getSession();

    if (error) {
        console.error('❌ [getAccessToken] Session error:', error);
        return null;
    }

    if (!session) {
        console.warn('⚠️ [getAccessToken] No session found - user may need to re-authenticate');
        return null;
    }

    console.log('✅ [getAccessToken] Session found');
    console.log('🔐 [getAccessToken] Session expires at:', new Date(session.expires_at! * 1000).toISOString());
    console.log('🔐 [getAccessToken] Token length:', session.access_token?.length);

    // Check if token is about to expire (within 60 seconds)
    const expiresIn = session.expires_at! - Math.floor(Date.now() / 1000);
    console.log('🔐 [getAccessToken] Token expires in:', expiresIn, 'seconds');

    if (expiresIn < 60) {
        console.warn('⚠️ [getAccessToken] Token expiring soon, attempting refresh...');
        const { data: refreshData, error: refreshError } = await supabase.auth.refreshSession();

        if (refreshError) {
            console.error('❌ [getAccessToken] Failed to refresh session:', refreshError);
